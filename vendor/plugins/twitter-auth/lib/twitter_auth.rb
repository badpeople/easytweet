module TwitterAuth
  ...
  def self.config(environment=Rails.env)
    @config ||= {}
    @config[environment] ||= YAML.load(File.open(Rails.root.to_s + '/config/twitter_auth.yml').read)[environment]
  end
  ...
  def self.consumer
    ...
    OAuth::Consumer.new(
      ENV['oauth_consumer_key'] || 'A8OwmRTmEnnDTA2oJystg',
      ENV['oauth_consumer_secret'] || 'brjR6f8NYftUlTRGCfOvWZfHmqhQ5NC2aXcn74yHa8',
      options 
    )
  end
